...
export default function ProfileScreen() {
  ...
  const fetchUsersReviews = async () => {
    try {
      ...
      const response = await fetch(`http://192.168.1.111:8765/review/get-reviews/user/${userId}`);
      const data = await response.json();
...
useEffect(() => {
  ...
  const addFavorite = async (userId, spotifyId, type) => {
    try {
      await axios.post("http://192.168.1.111:8765/favorite/add-favorite", {
        userId,
        spotifyId,
        type,
      });
...
const getUserFavoritesImages = async (accessToken, userId) => {
  try {
    ...
    const response = await axios.get(`http://192.168.1.111:8765/favorite/user/${userId}/all`);
...
// Aramayı debounce ile geciktirerek fazla istek göndermeyi engelle
...
}, 500); // 500ms içinde sadece 1 API isteği atılır
